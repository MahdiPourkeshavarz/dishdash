...
import { DirectionsMenu } from "../../map/DirectionMenu";
...
export const BotMessage = ({ message }: { message: ChatMessage }) => {
  const { theme } = useStore();
  return (
    <motion.div
    ...
    >
      ...
      <div
        className={`w-full p-3 rounded-2xl text-right rounded-bl-none space-y-3 ${
          theme === "dark"
          ...
          <div className="pt-3 border-t border-black/10 dark:border-white/10">
            <div className="flex flex-col gap-2">
              {message.places.map((place) => (
                <PlaceSuggestionCard key={place._id} place={place} />
              ))}
            </div>
          </div>
        )}
